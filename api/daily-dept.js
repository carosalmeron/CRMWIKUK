...
function buildEmailDept({ depKey, depConfig, nombre, incAyer, incNuevasAyer, incResueltasAyer, incPendientes, incEscaladas, tareasHoy, tareasVencidas, muestrasID, muestrasKOAyer, hitosAbiertos, proyectosActivos }) {
  ...
  const renderInc = (inc) => {
    ...
    return `<div style="border-left:3px solid ${pColor};padding:8px 0 8px 12px;margin-bottom:8px;background:#FAFAFA;border-radius:0 8px 8px 0">
      ...
      <p style="margin:3px 0 0;font-size:10px;color:#94A3B8">${inc.fecha || ""} · Reportada por ${inc.creadoPorNombre || nombreDe(inc.creadoPor || inc.agente)}</p>
    </div>`;
  };
...
export default async function handler(req, res) {
  ...
  try {
    const [usuarios, portalUsers, incidencias, tareas, muestras, proyectos] = await Promise.all([
      fbList("usuarios"), fbList("portal_users"), fbList("incidencias"), fbList("tareas"), fbList("muestras"), fbList("proyectos"),
    ]);

    // (v3.23.34) Construir nombreMap incluyendo portal_users
    const nombreMap = {};
    usuarios.forEach(u => {
      if (!u.nombre) return;
      ["id","_docId","grupoAgente","username","catalogoVendedor","portalId"].forEach(k=>{
        if(u[k]) nombreMap[String(u[k]).toUpperCase()] = u.nombre;
      });
      nombreMap[String(u.nombre).toUpperCase()] = u.nombre;
    });
    (portalUsers||[]).forEach(pu => {
      const nm = pu.nombre || pu.username || pu.id;
      if (!nm) return;
      ["id","_docId","catalogoVendedor","username","nombre"].forEach(k=>{
        if(pu[k]) nombreMap[String(pu[k]).toUpperCase()] = nm;
      });
    });
    const nombreDe = (id) => nombreMap[String(id||"").toUpperCase()] || id || "—";
