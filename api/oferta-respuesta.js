...
async function fbPatch(col, id, obj){
  const r = await fetch(`${FB}/${col}/${id}`, {
    method: "PATCH",
...
module.exports = async function handler(req, res){
  ...
  try{
    const body = req.body || {};
    const {ofertaId, token, tipo, lineaId, estado, contraoferta, cantidad, fechaEntrega, comentario} = body;
    ...
    await fbPatch("ofertas", ofertaId, updates);

    // Notificar al agente por email (best-effort, no bloquea respuesta)
    enviarNotificacionEmail(Object.assign({}, oferta, updates), body).catch(()=>{});
