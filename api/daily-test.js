...
module.exports = async function handler(req, res) {
  try {
    ...
    var hoy = new Date(), esViernes = hoy.getDay() === 5;
    // Forzar modo con ?modo=diario o ?modo=semanal
    var forzar = (req.query || {}).modo || '';
    if (forzar === 'diario') esViernes = false;
    if (forzar === 'semanal') esViernes = true;
    ...
    for (var i = 0; i < portal.length; i++) {
      ...
      try {
        if (rol === 'crm_agente' || rol === 'agente') {
          ...
          var myMPendAyer = mu.filter(function(x) { return !x.eliminada && x.estado === 'pendiente' && ism(x.agente, vd) && fechaMatch(x.fecha, ayer); });
          // Estrategias de AYER
          var myEstAyer = est.filter(function(x) { return !x.eliminada && ism(x.agente, vd) && fechaMatch(x.fecha, ayer); });
          var myEstEnCurso = est.filter(function(x) { return !x.eliminada && (x.estado === 'en_curso' || x.estado === 'pendiente_aprobacion') && ism(x.agente, vd); });
          // Incidencias de AYER
          var myIAyer = incDB.filter(function(x) { return !x.eliminada && (ism(x.agente, vd) || ism(x.autor, vd)) && fechaMatch(x.fecha, ayer); });
          var myIAbiertas = incDB.filter(function(x) { return !x.eliminada && x.estado !== 'cerrada' && x.estado !== 'resuelta' && (ism(x.agente, vd) || ism(x.autor, vd)); });
          // Tareas hechas ayer
          ...
          // Oportunidades con actividad ayer
          var myOActAyer = [];
          ...
          var myOGanAyer = ops.filter(function(x) { return !x.eliminada && (x.estado === 'ganada' || x.etapa === 'ganada') && (ism(x.agente, vd) || ism(x.agenteId, vd)) && fechaMatch(x.fechaCreacion || x.fecha, ayer); });
          ...
          if (esViernes) {
            ...
            myEDone.forEach(function(e) { logros.push('✅ Estrategia: <b>' + (e.cliente || '') + '</b>'); });
            myMDone.forEach(function(m) { logros.push('📦✅ <b>' + (m.prod || m.producto || '') + '</b>'); });
            myTDone.forEach(function(t) { logros.push('✅ <b>' + (t.titulo || t.texto || '') + '</b>'); });
            var pend = [];
            myHitos.forEach(function(h) { pend.push((h.vencido ? '⚠️' : '📍') + ' <b>' + h.hito + '</b> — ' + h.proy + (h.fecha ? ' · ' + (h.vencido ? '⚠️ VENCIDO ' : '📅 ') + h.fecha : '')); });
            myT.slice(0, 5).forEach(function(t) { 
              var tf = t.vence || t.fecha || '';
              var tfISO = tf.indexOf('/') >= 0 ? tf.split('/').reverse().join('-') : tf;
              if (tfISO && tfISO >= hoyISO && tfISO <= domingoStr) {
                pend.push('📌 <b>' + (t.titulo || t.texto || '') + '</b>' + (tf ? ' · 📅 ' + tf : ''));
              } else if (!tfISO) {
                pend.push('📌 <b>' + (t.titulo || t.texto || '') + '</b>');
              }
            });
            var sub = '📊 Resumen semanal ' + nm + ' — Semana ' + sem + ': ' + myVSem.length + ' visitas, ' + vPSem + ' pedidos';
            ...
            // ══ DIARIO ══
            var totalAyer = myVAyer.length + myOActAyer.length + myMDoneAyer.length + myMKOAyer.length + myMPendAyer.length + myEstAyer.length + myIAyer.length + myTDoneAyer.length;
            if (totalAyer === 0 && myT.length + myHitos.length + myEstEnCurso.length + myIAbiertas.length === 0) continue;
            // Visitas ayer
            ...
            // Logros ayer
            var lA = [];
            myOGanAyer.forEach(function(o) { lA.push('🏆 Opo ganada: <b>' + (o.nombre || o.cliente || '') + '</b>'); });
            myTDoneAyer.forEach(function(t) { lA.push('✅ Tarea: <b>' + (t.titulo || t.texto || '') + '</b>'); });
            // Muestras ayer
            var muestrasAyer = [];
            myMDoneAyer.forEach(function(m) { muestrasAyer.push('📦✅ OK: <b>' + (m.prod || m.producto || '') + '</b> — ' + (m.cliente || '')); });
            myMKOAyer.forEach(function(m) { muestrasAyer.push('📦❌ KO: <b>' + (m.prod || m.producto || '') + '</b> — ' + (m.cliente || '')); });
            myMPendAyer.forEach(function(m) { muestrasAyer.push('📦⏳ Enviada: <b>' + (m.prod || m.producto || '') + '</b> — ' + (m.cliente || '')); });
            // Oportunidades ayer
            var oA = myOActAyer.map(function(a) { return '💼 <b>' + a.nombre + '</b>' + (a.texto ? ' — ' + a.texto : ''); });
            // Estrategias ayer
            var estAyer = myEstAyer.map(function(e) { return '🎯 <b>' + (e.cliente || e.clienteNombre || '') + '</b>: ' + (e.texto || e.objetivo || '').substring(0, 60) + ' <span style="color:' + (e.estado === 'completada' ? '#22C55E' : '#F59E0B') + '">[' + (e.estado || 'nueva') + ']</span>'; });
            // Incidencias ayer
            var iA = myIAyer.map(function(x) { return '🚨 <b>' + (x.tipo || '') + '</b> ' + (x.clienteNombre || '') + ': ' + (x.descripcion || x.titulo || '').substring(0, 60); });
            // Pendiente semana: hitos + tareas
            var pend2 = [];
            myHitos.forEach(function(h) { pend2.push((h.vencido ? '⚠️' : '📍') + ' <b>' + h.hito + '</b> — ' + h.proy + (h.fecha ? ' · ' + (h.vencido ? '⚠️ VENCIDO ' : '📅 ') + h.fecha : '')); });
            myT.slice(0, 5).forEach(function(t) { 
              var tf = t.vence || t.fecha || '';
              var tfISO = tf.indexOf('/') >= 0 ? tf.split('/').reverse().join('-') : tf;
              if (tfISO && tfISO >= hoyISO && tfISO <= domingoStr) {
                pend2.push('📌 <b>' + (t.titulo || t.texto || '') + '</b>' + (tf ? ' · 📅 ' + tf : ''));
              } else if (!tfISO) {
                pend2.push('📌 <b>' + (t.titulo || t.texto || '') + '</b>');
              }
            });
            // Estrategias en curso
            var estPend = myEstEnCurso.map(function(e) { return '🎯 <b>' + (e.cliente || e.clienteNombre || '') + '</b>: ' + (e.texto || e.objetivo || '').substring(0, 50); });
            // Incidencias abiertas
            var incAbiertas = myIAbiertas.map(function(x) { return '🚨 <b>' + (x.tipo || '') + '</b> ' + (x.clienteNombre || '') + ': ' + (x.descripcion || x.titulo || '').substring(0, 60); });

            var sub = '📋 Informe diario ' + nm + ' — ' + ayerStr + ': ' + myVAyer.length + ' visitas';
            var body = hdr('#1E3A5F', '📋 Informe Diario — ' + nm, ayerStr) + '<div style="padding:20px 24px;border:1px solid #E2E8F0;border-top:none;border-radius:0 0 14px 14px">' +
              '<table width="100%" cellspacing="6" style="border-collapse:separate;margin-bottom:16px"><tr>' + kpi(myVAyer.length, 'Visitas ayer', '#22C55E') + kpi(vPAyer, 'Pedidos', '#3B82F6') + kpi(totalAyer, 'Gestiones', '#F59E0B') + kpi(myVSem.length, 'Sem. acum.', '#1E3A5F') + '</tr></table>' +
              blk('#F0FDF4', '#22C55E', 'Visitas ayer (' + myVAyer.length + ')', vlA) +
              blk('#EFF6FF', '#1E3A5F', 'Oportunidades ayer (' + myOActAyer.length + ')', oA) +
              blk('#F5F3FF', '#7C3AED', 'Muestras ayer (' + muestrasAyer.length + ')', muestrasAyer) +
              blk('#FFF7ED', '#F59E0B', 'Estrategias ayer (' + myEstAyer.length + ')', estAyer) +
              blk('#FEE2E2', '#EF4444', 'Incidencias ayer (' + myIAyer.length + ')', iA) +
              blk('#DBEAFE', '#3B82F6', 'Logros ayer (' + lA.length + ')', lA) +
              sep('⏳ Pendiente esta semana') +
              blk('#FFF7ED', '#F59E0B', 'Hitos + Tareas pendientes', pend2) +
              blk('#DBEAFE', '#3B82F6', 'Estrategias en curso (' + myEstEnCurso.length + ')', estPend) +
              blk('#FEE2E2', '#EF4444', 'Incidencias abiertas (' + myIAbiertas.length + ')', incAbiertas) +
              blk('#F5F3FF', '#7C3AED', 'Pipeline (' + myO.length + ' — ' + pp + '€)', myO.slice(0, 3).map(function(x) { return '💼 <b>' + (x.nombre || x.cliente || '') + '</b> — ' + (parseInt(x.valor) || 0) + '€'; })) +
              ft + '</div>';
            queue.unshift({ to: pu.email, subject: sub, html: '<div style="font-family:sans-serif;max-width:560px;margin:0 auto;background:#fff;border-radius:14px;overflow:hidden">' + body + '</div>' });
          }
          ...
          var tituloDir = esViernes ? '📊 Resumen Semanal — Dirección' : '📋 Informe Diario — Dirección';
          var subDir = esViernes ? '📊 Resumen semanal Dirección — Semana ' + sem + ': ' + aV.length + ' visitas' : '📋 Informe diario Dirección — ' + ayerStr;
          var vBlocks = '';
          ...
          var body2 = hdr('#0F172A', tituloDir, (esViernes ? 'Semana ' + sem + ' — ' + hoyStr : ayerStr)) + '<div style="padding:20px 24px;border:1px solid #E2E8F0;border-top:none;border-radius:0 0 14px 14px"><table width="100%" cellspacing="6" style="border-collapse:separate;margin-bottom:16px"><tr>' + kpi(visSource.length, esViernes ? 'Visitas sem.' : 'Visitas ayer', '#22C55E') + kpi(aI.length, 'Incidencias', '#EF4444') + kpi(aP + '€', 'Pipeline', '#3B82F6') + kpi(aE.length, 'Estrategias', '#7C3AED') + '</tr></table>' + sep('👥 Por vendedor') + vBlocks + blk('#DBEAFE', '#3B82F6', 'Estrategias en curso (' + aE.length + ')', aE.slice(0, 5).map(function(e) { return '🎯 <b>' + (e.cliente || e.clienteNombre || '') + '</b>: ' + (e.texto || e.objetivo || '').substring(0, 60); })) + blk('#FEE2E2', '#EF4444', 'Incidencias abiertas (' + aI.length + ')', aI.slice(0, 5).map(function(x) { return '🚨 <b>' + (x.tipo || '') + '</b> ' + (x.clienteNombre || '') + ': ' + (x.descripcion || x.titulo || '').substring(0, 60); })) + ft + '</div>';
    ...
    for (var q = 0; q < queue.length; q++) {
      try { await tr.sendMail({ from: process.env.SMTP_FROM || process.env.SMTP_USER, to: queue[q].to, subject: queue[q].subject, html: queue[q].html }); sent++; if (q < queue.length - 1) await new Promise(function(r) { setTimeout(r, 2000); }); } catch (ee) { errors.push(queue[q].to + ':' + ee.message); }
    }
    return res.status(200).json({ ok: true, sent: sent, queued: queue.length, tipo: esViernes ? 'semanal' : 'diario', ayerFecha: ayer.toLocaleDateString('es-ES', {day:'2-digit',month:'2-digit',year:'2-digit'}), subjects: queue.map(function(q) { return q.subject; }), errors: errors });
