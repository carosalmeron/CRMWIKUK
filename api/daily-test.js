      ...
      try{
        if(rol==='crm_agente'||rol==='agente'){
          ...
          if(esViernes){
            ...
            var logros=[];myOGan.forEach(function(o){logros.push('🏆 <b>'+(o.nombre||o.cliente||'')+'</b>');});myEDone.forEach(function(e){logros.push('✅ <b>'+(e.cliente||'')+'</b>');});myMDone.forEach(function(m){logros.push('📦✅ <b>'+(m.prod||m.producto||'')+'</b>');});myTDone.forEach(function(t){logros.push('✅ <b>'+(t.titulo||t.texto||'')+'</b>');});
            var pend=[];myHitos.forEach(function(h){pend.push((h.vencido?'⚠️':'📍')+' <b>'+h.hito+'</b> — '+h.proy);});myT.slice(0,5).forEach(function(t){pend.push('📌 <b>'+(t.titulo||t.texto||'')+'</b>');});
