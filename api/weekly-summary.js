...
async function fbList(col) {
  ...
  do {
    ...
    // (v3.23.34) Preservar _docId siempre, incluso si el documento tiene un field "id" interno
...
// (v3.16.0) Para oportunidades: incluir si fueron CREADAS o GESTIONADAS esta semana
function opEnSemana(o, sem, lu, vi) {
  if (enSemana(o, sem, lu, vi)) return true;
  // Cierre esta semana
  if (o.fechaCierre) { const d = parseFecha(o.fechaCierre); if (d && d >= lu && d <= vi) return true; }
  // Actividad esta semana
  if ((o.actividad||[]).some(a => { const d = parseFecha(a && a.fecha); return d && d >= lu && d <= vi; })) return true;
  // Historial esta semana
  if ((o.historial||[]).some(c => { const d = parseFecha(c && c.fecha); return d && d >= lu && d <= vi; })) return true;
  // Feedback esta semana
  if (o.fechaFeedback) { const d = parseFecha(o.fechaFeedback); if (d && d >= lu && d <= vi) return true; }
  return false;
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const [usuarios, portalUsers, visitas, oportunidades, estrategias, incidencias, muestras, informes, presupuesto, monetizacion, proyectos, clientes, planes, agentesKpi, ofertas] = await Promise.all([
    ...
    // (v3.23.34) Fusionar portal_users con usuarios CRM
    const indexUsuarios = {};
    ...
    (portalUsers || []).forEach(pu => {
      ...
      if (existeIdx >= 0) {
        ...
        usuarios.push({
          id: pu._docId || pu.id || pu.username || pu.nombre,
          _docId: pu._docId,
    ...
    // (v3.23.34) Override de equipos para usuarios sin equipo conocido
    const EQUIPOS_OVERRIDE = {
      "JOSELUIS": "WIKUK", "JLGARCIA": "WIKUK", "JL GARCIA": "WIKUK", "JLGARCÍA": "WIKUK",
      "ESTHER": "WIKUK", "MIGUEL": "WIKUK",
    };
    usuarios.forEach(u => {
      if (u.equipo) return;
    ...
    // (v3.23.34) Recolectar agentes en actividad para crear virtuales
    const agentesEnActividad = new Set();
    const recogerAgente = (item) => {
      [item.agente, item.agenteId, item.autor, item.creadoPor].forEach(a => {
        if (a) agentesEnActividad.add(String(a).toUpperCase().trim());
      });
    };
    [...visitas, ...oportunidades, ...estrategias, ...incidencias, ...muestras, ...(ofertas||[])].forEach(recogerAgente);
    ...
    agentesEnActividad.forEach(agId => {
      ...
      let nombre = agId, equipoOverride = EQUIPOS_OVERRIDE[agId] || "", catalogoVendedor = agId;
      ...
      usuarios.push({
        id: agId.toLowerCase(), _docId: agId, nombre, rol: "agente",
        equipo: equipoOverride, catalogoVendedor, portalDocId: agId, _virtual: true,
      });
    });
    ...
    const opsSemana = noElim(oportunidades).filter(x => opEnSemana(x, semana, lu, vi));
    ...
    function idsAg(ag) {
      const ids = new Set();
      // (v3.23.34) Más permisivo - incluir todos los identificadores posibles + aliases
    ...
    // level: "agente" | "resp" | "dir" | "ceo"
    //   agente / resp → sin filtro (ven todo lo de su scope)
    //   dir  → solo aprobado por el resp (aprobadoResp !== false)
    //   ceo  → aprobado por resp Y por dir (aprobadoResp !== false && aprobadoJefe !== false)
    // Importante: usamos !== false para que las visitas/ops sin marcar se incluyan POR DEFECTO.
    function aplicarAprobacion(arr, level) {
      if (level === "dir") {
        return arr.filter(x => x.aprobadoResp !== false);
      }
      if (level === "ceo") {
        return arr.filter(x => x.aprobadoResp !== false && x.aprobadoJefe !== false);
      }
      return arr; // agente / resp / cualquier otro → sin filtro
    }

    // ── Build context for a scope of agents ──
    function buildCtx(titulo, mgNombre, agentesScope, esJefe, aprobacionLevel) {
      // Aplicar filtro de aprobación a visitas y oportunidades del scope
      const visSemFiltrado = aplicarAprobacion(visSem, aprobacionLevel);
      const opsSemanaFiltrado = aplicarAprobacion(opsSemana, aprobacionLevel);
      const opsResueltasFiltrado = aplicarAprobacion(opsResueltas, aprobacionLevel);
      const opsNuevasSemFiltrado = aplicarAprobacion(opsNuevasSem, aprobacionLevel);
      ...
      const porVendedor = agentesScope.map(ag => {
        ...
        return {
          ...
          vis:sf(visSemFiltrado).filter(x=>pertA(x,ids)).length, visR,
          ops:sf(opsSemanaFiltrado).filter(x=>pertA(x,ids)).length,
      ...
      if (!esJefe) {
        porEquipo = ["WIKUK","INTERKEY"].map(eq => {
          ...
          return {
            ...
            vis:sf(visSemFiltrado).filter(x=>pertA(x,idsEq)).length, visR:calcVisRuta(idsEq),
            ops:sf(opsSemanaFiltrado).filter(x=>pertA(x,idsEq)).length,
      ...
      return {
        ...
        totales: {
          ...
          visitasReal:sf(visSemFiltrado).length, visitasRuta:totalVisR,
          ...
          oportunidades:sf(opsSemanaFiltrado).length + opsResueltasFiltrado.filter(o=>pertA(o,allIds)||!esJefe).length,
        },
        porEquipo, porVendedor,
        porDia: {
          visitas: groupByDay(sf(visSemFiltrado), "fecha"),
          ...
          opsDone: opsResueltasFiltrado.filter(o=>pertA(o,allIds)||!esJefe),
          opsNew: opsNuevasSemFiltrado.filter(o=>pertA(o,allIds)||!esJefe),
        },
      };
    }
    ...
    for (const mg of managers) {
      ...
      // (v3.23.34) Roles activos ampliado + cualquier usuario con catalogoVendedor o virtual
      ...
      const scope = usuarios.filter(u=>{
        ...
        if (esJefe) return u.equipo === mg.equipo || u.id === mg.id || u._virtual;
        return true;
      });
      // Nivel de aprobación según rol del destinatario:
      //   jefe (resp)    → sin filtro (ve todo de su equipo)
      //   director       → solo aprobadoResp !== false
      //   ceo            → solo aprobadoResp !== false Y aprobadoJefe !== false
    ...
    for (const ag of agentes) {
      const ctx = buildCtx("Tu cierre de semana", ag.nombre, [ag], true, "agente");
