...
export default async function handler(req, res) {
  ...
  try {
    ...
    const [usuarios, visitas, oportunidades, estrategias, muestras, proyectos, clientes] = await Promise.all([
    ...
    const mueDia = noElim(muestras).filter(x => enFecha(x, fecha));
    ...
    function buildCtx(titulo, mgNombre, agentesScope, esJefe, aprobacionLevel) {
      ...
      const porVendedor = agentesScope.map(ag => {
        ...
        return {
          ...
          ops: sf(opsNuevasFiltradas).filter(x => pertA(x, ids)).length + sf(opsResueltasFiltradas).filter(x => pertA(x, ids)).length,
