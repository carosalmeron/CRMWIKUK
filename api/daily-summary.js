...
try {
if(!process.env.SMTP_USER) return res.status(200).json({error:'no smtp'});
...
if(!vd)continue;
var myV=vis.filter(function(v){return parseInt(v.semana||0)===sem&&((v.agente||'').toUpperCase()===vd||(v.agenteId||'').toUpperCase()===vd);});
var myT=tareas.filter(function(t){return !t.eliminada&&t.estado!=='hecha'&&((t.agente||'').toUpperCase()===vd||(t.agenteId||'').toUpperCase()===vd);});
var myI=inc.filter(function(x){return !x.eliminada&&x.estado!=='cerrada'&&x.estado!=='resuelta'&&((x.agente||'').toUpperCase()===vd||(x.autor||'').toUpperCase()===vd);});
var myE=est.filter(function(x){return !x.eliminada&&x.estado==='en_curso'&&(x.agente||'').toUpperCase()===vd;});
var myO=ops.filter(function(x){return !x.eliminada&&['ganada','perdida','cerrada_ganada','cerrada_perdida'].indexOf(x.estado||x.etapa)===-1&&((x.agente||'').toUpperCase()===vd||(x.agenteId||'').toUpperCase()===vd);});
var myM=mu.filter(function(x){return !x.eliminada&&x.estado==='pendiente'&&(x.agente||'').toUpperCase()===vd;});
...
var am={};for(var j=0;j<aV.length;j++){var av=aV[j];var ak=(av.agente||av.agenteId||'?').toUpperCase();if(!am[ak])am[ak]={n:0,p:0};am[ak].n++;if(av.resultado==='pedido')am[ak].p++;}
...
if(!eq)continue;
var eV=portal.filter(function(p){return p.equipo===eq&&(p.rol==='crm_agente'||p.rol==='agente');}).map(function(p){return(p.catalogoVendedor||'').toUpperCase();}).filter(Boolean);
var tE=tareas.filter(function(t){return !t.eliminada&&t.estado!=='hecha'&&eV.indexOf((t.agente||'').toUpperCase())>=0;});
var iE=inc.filter(function(x){return !x.eliminada&&x.estado!=='cerrada'&&x.estado!=='resuelta';});
var vE=vis.filter(function(v){return parseInt(v.semana||0)===sem&&eV.indexOf((v.agente||v.agenteId||'').toUpperCase())>=0;});
sub='Equipo '+eq+' Sem.'+sem+': '+vE.length+' visitas';
body=hdr(eq==='WIKUK'?'#166534':'#92400E','Equipo '+eq,'Semana '+sem+' — '+hoyStr)+'<div style="padding:20px 24px;border:1px solid #E2E8F0;border-top:none;border-radius:0 0 14px 14px">'+'<table width="100%" cellspacing="6" style="border-collapse:separate;margin-bottom:16px"><tr>'+kpi(vE.length,'Visitas','#22C55E')+kpi(tE.length,'Tareas','#F59E0B')+kpi(iE.length,'Incidencias','#EF4444')+'</tr></table>'+blk('#FEF3C7','#F59E0B','Tareas',tE.slice(0,5).map(function(t){return t.titulo||'';}))+ft+'</div>';
}else{continue;}
if(!sub||!body)continue;
await tr.sendMail({from:process.env.SMTP_FROM||process.env.SMTP_USER,to:pu.email,subject:sub,html:'<div style="font-family:sans-serif;max-width:560px;margin:0 auto;background:#fff;border-radius:14px;overflow:hidden">'+body+'</div>'});
sent++;
}catch(ee){errors.push((pu.email||'?')+':'+ee.message);}
}
