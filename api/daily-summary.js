...
function renderResumenAg(label, d) {
  ...
  if (d.inc > 0) partes.push(`<span style="background:#FEE2E2;padding:3px 8px;border-radius:6px;font-size:11px;font-weight:700;color:#991B1B">🚨 ${d.inc} incidencia${d.inc>1?"s":""}</span>`);
...
function buildEmail(ctx) {
  ...
  html += `<p style="${S.hdr}">📊 Actividad del día</p>
    <div style="display:flex;gap:5px;flex-wrap:wrap;margin-bottom:16px">
      ...
      ${(totales.incidenciasNuevas||0)>0?kpiSimple(totales.incidenciasNuevas,"Incidencias","#991B1B"):""}
      ${(totales.incRespNueva||0)>0?kpiSimple(totales.incRespNueva,"Resp. inc.","#3B82F6"):""}
    </div>`;
  ...
  // (v3.23.59) 4b. INCIDENCIAS DEL DÍA
  const incList = (datos && datos.incDia) || [];
  if (incList.length > 0) {
    html += `<p style="${S.hdr}">🚨 Incidencias del día (${incList.length})</p>`;
    incList.forEach(inc => {
      const tipo = inc.tipo || "otro";
      const tipoLbl = {
        calidad:"⚠️ Calidad", logistica:"🚚 Logística", produccion:"🏭 Producción",
        comercial:"💼 Comercial", administracion:"📄 Administración",
        coordinacion:"🧭 Coordinación", stock:"📦 Stock", id:"🔬 I+D", otro:"📌 Otro"
      }[tipo] || tipo;
      const prio = inc.prioridad === "urgente" ? "🔴 URGENTE" :
                   inc.prioridad === "alta" ? "🟠 Alta" :
                   inc.prioridad === "media" ? "🟡 Media" : "🟢 Baja";
      const colorBar = inc.prioridad === "urgente" || inc.prioridad === "alta" ? "#DC2626" :
                       inc.prioridad === "media" ? "#F59E0B" : "#22C55E";
      const cliente = inc.cliente || inc.clienteNombre || "—";
      const desc = inc.descripcion || inc.titulo || inc.asunto || "(sin descripción)";
      const autor = nombreMap[String(inc.autor || inc.agente || "").toUpperCase()] || inc.autor || inc.agente || "";
      const estado = inc.estado === "resuelta" ? "✅ Resuelta" :
                     inc.estado === "cerrada" ? "⬜ Cerrada" :
                     inc.estado === "en_proceso" ? "🟡 En proceso" : "🔴 Abierta";
      html += `<div style="border-left:4px solid ${colorBar};padding:6px 0 6px 12px;margin-bottom:6px;background:#FEF2F2;border-radius:0 8px 8px 0">
        <p style="margin:0;font-size:12px"><b>${cliente}</b> <span style="color:#64748B">— ${autor}</span></p>
        <p style="margin:2px 0 0;font-size:12px;color:#475569">${tipoLbl} · ${prio} · ${estado}</p>
        <p style="margin:2px 0 0;font-size:12px;color:#1E293B">${desc.substring(0, 200)}${desc.length > 200 ? "…" : ""}</p>
      </div>`;
    });
  }
  ...
  // (v3.23.32) Sección Respuestas a tus incidencias
  const incRN = (ctx.datos && ctx.datos.incRespNueva) || [];
  if (incRN.length > 0) {
    html += `<p style="${S.hdr}">💬 Respuestas a tus incidencias (${incRN.length} sin leer)</p>`;
    incRN.slice(0, 10).forEach(inc => {
      const hist = inc.historialEscalado || [];
      let ult = null;
      const miNombre = ctx.nombre || "";
      for (let k = hist.length-1; k >= 0; k--) {
        if (hist[k].por && hist[k].por !== miNombre) { ult = hist[k]; break; }
      }
      const titulo = inc.titulo || inc.descripcion || inc.tipo || "Incidencia";
      const accion = ult ? ult.accion : "actividad nueva";
      const por = ult ? ult.por : "";
      const fecha = ult && ult.fecha ? " · " + ult.fecha : "";
      const nota = ult && ult.nota ? `<p style="margin:2px 0 0 18px;font-size:11px;color:#475569;font-style:italic">💬 ${ult.nota.substring(0, 200)}${ult.nota.length > 200 ? "…" : ""}</p>` : "";
      html += `<div style="padding:4px 0 6px 10px;margin-bottom:4px;border-left:3px solid #3B82F6"><p style="margin:0;font-size:12px"><b>${titulo}</b> <span style="color:#64748B">— ${accion} por ${por}${fecha}</span></p>${nota}</div>`;
    });
  }
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const [usuarios, portalUsers, visitas, oportunidades, estrategias, muestras, proyectos, clientes, ofertas, incidencias] = await Promise.all([
      ...
      fbList("ofertas"), fbList("incidencias"),
    ]);
    ...
    // Map ID → nombre (se reconstruye después de virtuales/aliases)
    let nombreMap = {};
    ...
    // (v3.23.59) Incidencias creadas en el día
    const incDia = noElim(incidencias).filter(x => enFecha(x, fecha));
    ...
    function idsAg(ag) {
      const ids = new Set();
      // (v3.23.8/v3.23.10/v3.23.13/v3.23.14) Más permisivo - incluir todos los identificadores posibles + aliases
      ...
      if (ag._aliases && Array.isArray(ag._aliases)) {
        ag._aliases.forEach(a => { if (a) ids.add(String(a).toUpperCase()); });
      }
      return ids;
    }
    ...
    function buildCtx(titulo, mgNombre, agentesScope, esJefe, aprobacionLevel) {
      ...
      const porVendedor = agentesScope
        ...
        return {
          ...
          inc: sf(incDia).filter(x => pertA(x, ids)).length,
        };
      ...
      if (!esJefe) {
        porEquipo = ["WIKUK","INTERKEY"].map(eq => {
          ...
          return {
            ...
            inc: sf(incDia).filter(x => pertA(x, idsEq)).length,
          };
        });
      }
      ...
      // (v3.23.32) Incidencias del manager con respuesta nueva sin leer
      const incRespNueva = (incidencias||[]).filter(inc => {
        if (inc.eliminada) return false;
        // El manager es destinatario si fue creador o está informado
        const idsMg = idsAg(esJefe ? agentesScope[0] : {id:"ceo",_docId:"ceo"}); // manager
        // En realidad mejor: las incidencias mías personalmente
        const miId = (esJefe ? agentesScope[0].id : aprobacionLevel === "agente" ? agentesScope[0].id : "ceo");
        if (inc.autor !== miId && !(inc.informadoA||[]).includes(miId)) return false;
        const hist = inc.historialEscalado||[];
        if (hist.length===0) return false;
        // Última actividad por otro
        let ultPorOtro = null;
        const miNombre = esJefe ? (agentesScope[0].nombre||"") : (mgNombre||"");
        for (let k=hist.length-1; k>=0; k--) {
          if (hist[k].por && hist[k].por !== miNombre) { ultPorOtro = hist[k]; break; }
        }
        if (!ultPorOtro) return false;
        // ¿Visto?
        const visto = inc[`lecturas_${miId}`];
        if (!visto) return true;
        if (!ultPorOtro.fecha) return false;
        const pH = ultPorOtro.fecha.split("/");
        if (pH.length<3) return false;
        const fAct = new Date(pH[2].length===2?"20"+pH[2]:pH[2], pH[1]-1, pH[0]).getTime();
        const fVis = new Date(visto).getTime();
        return fAct > fVis;
      });
      ...
      return {
        ...
        totales: {
          ...
          incidenciasNuevas: sf(incDia).length,
          incRespNueva: incRespNueva.length,
        },
    ...
    // (v3.23.14) Antes de crear virtual, buscar match fuzzy por tokens (ej. "JL" matchea con nombre "JL GARCÍA")
    const usuariosVirtualesCreados = [];
    const aliasesAplicados = [];
    agentesEnActividad.forEach(agId => {
      if (!agId || agId === "—" || agId === "UNDEFINED") return;
      if (idsCubiertos.has(agId)) return;

      // (v3.23.14) Fuzzy match por tokens (separados por espacio/guion/underscore)
      // Ej: agId "JL" matchea con nombre "JL GARCÍA" (token "JL" está)
      let userMatch = null;
      for (const u of usuarios) {
        const camposChequear = ["id","_docId","grupoAgente","nombre","username","catalogoVendedor","portalId","portalDocId"];
        for (const k of camposChequear) {
          if (!u[k]) continue;
          const tokens = String(u[k]).toUpperCase().trim().split(/[\s\-_]+/).filter(Boolean);
          if (tokens.includes(agId)) { userMatch = u; break; }
        }
        if (userMatch) break;
      }
      if (userMatch) {
        // Añadir agId como alias del usuario existente
        if (!userMatch._aliases) userMatch._aliases = [];
        userMatch._aliases.push(agId);
        idsCubiertos.add(agId);
        aliasesAplicados.push(`${agId}→${userMatch.nombre || userMatch.id}`);
        return;
      }
    ...
    if (aliasesAplicados.length > 0) {
      console.log(`[DAILY] (v3.23.14) Aliases fuzzy aplicados: ${JSON.stringify(aliasesAplicados)}`);
    }
    ...
    // (v3.23.15) Reconstruir nombreMap AHORA con todos los usuarios (incl. virtuales y aliases)
    nombreMap = {};
    usuarios.forEach(u => {
      if (u.nombre) {
        nombreMap[String(u.id||"").toUpperCase()] = u.nombre;
        if (u._docId) nombreMap[String(u._docId).toUpperCase()] = u.nombre;
        ...
        if (u.portalDocId) nombreMap[String(u.portalDocId).toUpperCase()] = u.nombre;
        if (u._aliases && Array.isArray(u._aliases)) {
          u._aliases.forEach(a => { if (a) nombreMap[String(a).toUpperCase()] = u.nombre; });
        }
