...
var myMAll=mu.filter(function(x){return !x.eliminada&&x.estado==='pendiente'&&ism(x.agente,vd);});
...
var logros=[];
if(myOGan.length>0)myOGan.forEach(function(o){logros.push('🏆 Oportunidad ganada: <b>'+(o.nombre||o.cliente||'')+'</b>'+(o.valor?' — '+o.valor+'€':''));});
if(myEDone.length>0)myEDone.forEach(function(e){logros.push('✅ Estrategia completada: <b>'+(e.cliente||e.clienteNombre||'')+'</b>');});
...
if(myTDone.length>0)myTDone.forEach(function(t){logros.push('✅ Tarea hecha: <b>'+(t.titulo||t.texto||'')+'</b>');});
var pendItems=[];
...
var body=hdr('#1E3A5F','Buenos dias, '+nm,'Semana '+sem+' — '+hoyStr)+'<div style="padding:20px 24px;border:1px solid #E2E8F0;border-top:none;border-radius:0 0 14px 14px">'+'<table width="100%" cellspacing="6" style="border-collapse:separate;margin-bottom:16px"><tr>'+kpi(myV.length,'Visitas','#22C55E')+kpi(vP,'Pedidos','#3B82F6')+kpi(myT.length,'Tareas','#F59E0B')+kpi(pp+'€','Pipeline','#1E3A5F')+'</tr></table>'+blk('#F0FDF4','#22C55E','Visitas semana '+sem+' ('+myV.length+')',vl)+blk('#DBEAFE','#3B82F6','Logros esta semana ('+logros.length+')',logros)+blk('#FFF7ED','#F59E0B','Pendiente — '+myHitos.length+' hitos, '+myT.length+' tareas',pendItems.concat(myT.slice(0,5).map(function(t){return '📌 <b>'+(t.titulo||t.texto||'')+'</b>'+(t.vence?' — Vence: '+t.vence:'');})))+blk('#FEE2E2','#EF4444','Incidencias abiertas ('+myI.length+')',myI.slice(0,4).map(function(x){return '<b>'+(x.tipo||'')+'</b> '+(x.clienteNombre||'')+': '+(x.descripcion||x.titulo||'').substring(0,80);}))+blk('#DBEAFE','#3B82F6','Estrategias en curso ('+myE.length+')',myE.slice(0,4).map(function(x){return '<b>'+(x.cliente||x.clienteNombre||'')+'</b>: '+(x.texto||x.objetivo||'').substring(0,80);}))+blk('#F5F3FF','#7C3AED','Oportunidades activas ('+myO.length+')',myO.slice(0,4).map(function(x){return '<b>'+(x.nombre||x.cliente||'')+'</b> — '+(parseInt(x.valor)||0)+'€ ('+(x.prob||0)+'%)';}))+blk('#F5F3FF','#7C3AED','Muestras pendientes semana ('+myMPend.length+')'+(myMAll.length>myMPend.length?' — '+myMAll.length+' total':''),myMPend.slice(0,5).map(function(x){return '<b>'+(x.prod||x.producto||'')+'</b> — '+(x.cliente||x.clienteNombre||'');}))+ft+'</div>';
...
var aV=vis.filter(function(v){return parseInt(v.semana||0)===sem;});var aT=tareas.filter(function(t){return !t.eliminada&&t.estado!=='hecha';});var aI=inc.filter(function(x){return !x.eliminada&&x.estado!=='cerrada'&&x.estado!=='resuelta';});var aO=ops.filter(function(o){return !o.eliminada&&['ganada','perdida'].indexOf(o.estado||o.etapa)===-1;});var aP=aO.reduce(function(s,o){return s+(parseInt(o.valor)||0);},0);
