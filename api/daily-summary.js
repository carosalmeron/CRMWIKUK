...
queue.unshift({to:pu.email,subject:'Actividad '+nm+' Sem.'+sem+': '+myV.length+' visitas, '+vP+' pedidos',html:'<div style="font-family:sans-serif;max-width:560px;margin:0 auto;background:#fff;border-radius:14px;overflow:hidden">'+hdr('#1E3A5F','Buenos dias, '+nm,'Semana '+sem+' — '+hoyStr)+'<div style="padding:20px 24px;border:1px solid #E2E8F0;border-top:none;border-radius:0 0 14px 14px">'+'<table width="100%" cellspacing="6" style="border-collapse:separate;margin-bottom:16px"><tr>'+kpi(myV.length,'Visitas','#22C55E')+kpi(vP,'Pedidos','#3B82F6')+kpi(myT.length,'Tareas','#F59E0B')+kpi(pp+'€','Pipeline','#1E3A5F')+'</tr></table>'+blk('#F0FDF4','#22C55E','Visitas semana '+sem,vl)+blk('#FEF3C7','#F59E0B','Tareas ('+myT.length+')',myT.slice(0,5).map(function(t){return '<b>'+(t.titulo||t.texto||'')+'</b>';}))+blk('#FEE2E2','#EF4444','Incidencias ('+myI.length+')',myI.slice(0,4).map(function(x){return '<b>'+(x.tipo||'')+'</b> '+(x.clienteNombre||'');}))+blk('#DBEAFE','#3B82F6','Estrategias ('+myE.length+')',myE.slice(0,4).map(function(x){return '<b>'+(x.cliente||x.clienteNombre||'')+'</b>';}))+blk('#F5F3FF','#7C3AED','Muestras ('+myM.length+')',myM.slice(0,4).map(function(x){return '<b>'+(x.prod||x.producto||'')+'</b> '+(x.cliente||'');}))+ft+'</div></div>'});
...
var am={};for(var j=0;j<aV.length;j++){var av=aV[j];var ak=rv(av.agente||av.agenteId||'?');if(!am[ak])am[ak]={n:0,p:0};am[ak].n++;if(av.resultado==='pedido')am[ak].p++;}
