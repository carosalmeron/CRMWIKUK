...
export default async function handler(req, res) {
  ...
  try {
    ...
    // (v3.23.8/v3.23.10) Mezclar portal_users con usuarios para captar Esther, JLGarcia, Miguel y demás
    ...
    usuarios.forEach((u, i) => {
      ["id","grupoAgente","username","catalogoVendedor","nombre"].forEach(k => {
    ...
    (portalUsers || []).forEach(pu => {
      ...
      const claves = [];
      ["id","catalogoVendedor","username","nombre"].forEach(k => {
      ...
      if (existeIdx >= 0) {
        ...
        // Añadir el id portal al index para que pertA matchee
        ...
        usuarios.push({
          ...
          portalId: pu.id || "", // id técnico del portal (u_xxx) para matching
          _fromPortal: true,
        });
    ...
    function idsAg(ag) {
      const ids = new Set();
      // (v3.23.8/v3.23.10) Más permisivo - incluir todos los identificadores posibles
      ["id","grupoAgente","nombre","username","catalogoVendedor","portalId"].forEach(k => { if (ag[k]) ids.add(String(ag[k]).toUpperCase()); });
      return ids;
    }
    ...
    [...visDia, ...opsNuevasDia, ...opsResueltasDia, ...opsGestionadasDia,
     ...ofNuevasDia, ...ofResueltasDia, ...mueDia, ...estDia].forEach(recogerAgente);

    // Comprobar cuáles ya están cubiertos por algún usuario existente
    const idsCubiertos = new Set();
    usuarios.forEach(u => {
      ["id","grupoAgente","nombre","username","catalogoVendedor"].forEach(k => {
    ...
    agentesEnActividad.forEach(agId => {
      ...
      // Saltar IDs técnicos del portal (u_xxx) que no resuelven a nadie
      if (agId.startsWith("U_")) return;
      // Crear usuario virtual
      const nombre = agId; // el id en sí ya es algo legible (ej "ESTHER", "MIGUEL")
      usuarios.push({
        id: agId.toLowerCase(),
        nombre: nombre,
        rol: "agente",
        equipo: "", // sin equipo conocido
        catalogoVendedor: agId,
        _virtual: true,
      });
      usuariosVirtualesCreados.push(agId);
    });
    if (usuariosVirtualesCreados.length > 0) {
      console.log(`[DAILY] (v3.23.10) Usuarios virtuales creados desde actividad: ${JSON.stringify(usuariosVirtualesCreados)}`);
    }
