...
if(!emailBodies[pu.email])emailBodies[pu.email]={subject:sub,parts:[]};
emailBodies[pu.email].parts.push(body);
if(emailBodies[pu.email].parts.length>1)emailBodies[pu.email].subject='CRM Sem.'+sem+' — Resumen completo';
}catch(ee){}}
var sent=0,errors=[];
...
try{
await tr.sendMail({from:process.env.SMTP_FROM||process.env.SMTP_USER,to:em,subject:eb.subject,html:'<div style="font-family:sans-serif;max-width:560px;margin:0 auto">'+eb.parts.join('<div style="height:24px"></div>')+'</div>'});
sent++;
