...
var hoy=new Date();var dow=hoy.getDay();var esViernes=dow===5;
...
var myOActAyer=[];
myO.forEach(function(o){(o.actividad||[]).forEach(function(a){if(fechaMatch(a.fecha,ayer))myOActAyer.push({nombre:o.nombre||o.cliente||’’,texto:a.texto||’’,tipo:a.tipo||’’});});});
var myIAyer=inc.filter(function(x){return !x.eliminada&&(ism(x.agente,vd)||ism(x.autor,vd))&&fechaMatch(x.fecha,ayer);});
var vPAyer=myVAyer.filter(function(v){return v.resultado===‘pedido’;}).length;
if(esViernes){
if(myVSem.length+myT.length+myI.length+myE.length+myO.length+myMPend.length+myHitos.length+myMDone.length+myEDone.length+myOGan.length===0)continue;
var vl=myVSem.map(function(v){var ic=v.resultado===‘pedido’?‘✅’:v.resultado===‘llamada’?‘📞’:v.resultado===‘no_contesta’?‘📵’:‘👋’;var nt=v.notas||v.nota||’’;return ic+’ <b>’+(v.clienteNombre||v.cliente||’’)+’</b>’+(nt?’<br/><span style="color:#64748B;font-size:11px;padding-left:24px">’+nt+’</span>’:’’);});
var logros=[];
if(myOGan.length>0)myOGan.forEach(function(o){logros.push(‘🏆 Opo ganada: <b>’+(o.nombre||o.cliente||’’)+’</b>’+(o.valor?’ — ‘+o.valor+‘€’:’’));});
if(myEDone.length>0)myEDone.forEach(function(e){logros.push(‘✅ Estrategia: <b>’+(e.cliente||e.clienteNombre||’’)+’</b>’);});
if(myMDone.length>0)myMDone.forEach(function(m){logros.push(‘📦✅ Muestra OK: <b>’+(m.prod||m.producto||’’)+’</b>’);});
if(myMKO.length>0)myMKO.forEach(function(m){logros.push(‘📦❌ Muestra KO: <b>’+(m.prod||m.producto||’’)+’</b>’);});
...
var sub=‘📊 Resumen semanal ‘+nm+’ Sem.’+sem+’: ‘+myVSem.length+’ visitas, ‘+vPSem+’ pedidos’;
var body=hdr(’#1E3A5F’,‘📊 Resumen Semanal — ‘+nm,‘Semana ‘+sem+’ — ‘+hoyStr)+’<div style="padding:20px 24px;border:1px solid #E2E8F0;border-top:none;border-radius:0 0 14px 14px">’+’<table width="100%" cellspacing="6" style="border-collapse:separate;margin-bottom:16px"><tr>’+kpi(myVSem.length,‘Visitas’,’#22C55E’)+kpi(vPSem,‘Pedidos’,’#3B82F6’)+kpi(myT.length,‘Tareas pend.’,’#F59E0B’)+kpi(pp+‘€’,‘Pipeline’,’#1E3A5F’)+’</tr></table>’+blk(’#F0FDF4’,’#22C55E’,‘Visitas semana ‘+sem+’ (’+myVSem.length+’)’,vl)+blk(’#DBEAFE’,’#3B82F6’,‘Logros esta semana (’+logros.length+’)’,logros)+blk(’#FFF7ED’,’#F59E0B’,‘Pendiente — ‘+myHitos.length+’ hitos, ‘+myT.length+’ tareas’,pendItems.concat(myT.slice(0,5).map(function(t){return ‘📌 <b>’+(t.titulo||t.texto||’’)+’</b>’+(t.vence?’ — ‘+t.vence:’’);})))+blk(’#FEE2E2’,’#EF4444’,‘Incidencias (’+myI.length+’)’,myI.slice(0,4).map(function(x){return ‘<b>’+(x.tipo||’’)+’</b> ‘+(x.clienteNombre||’’)+’: ‘+(x.descripcion||x.titulo||’’).substring(0,80);}))+blk(’#DBEAFE’,’#3B82F6’,‘Estrategias (’+myE.length+’)’,myE.slice(0,4).map(function(x){return ‘<b>’+(x.cliente||x.clienteNombre||’’)+’</b>: ‘+(x.texto||x.objetivo||’’).substring(0,80);}))+blk(’#F5F3FF’,’#7C3AED’,‘Oportunidades (’+myO.length+’)’,myO.slice(0,4).map(function(x){return ‘<b>’+(x.nombre||x.cliente||’’)+’</b> — ‘+(parseInt(x.valor)||0)+‘€’;}))+blk(’#F5F3FF’,’#7C3AED’,‘Muestras pendientes (’+myMPend.length+’)’,myMPend.slice(0,5).map(function(x){return ‘<b>’+(x.prod||x.producto||’’)+’</b> — ‘+(x.cliente||’’);}))+ft+’</div>’;
...
var totalAyer=myVAyer.length+myOActAyer.length+myEDoneAyer.length+myMDoneAyer.length+myMKOAyer.length+myIAyer.length;
if(totalAyer===0&&myT.length+myHitos.length===0)continue;
var vlAyer=myVAyer.map(function(v){var ic=v.resultado===‘pedido’?‘✅’:v.resultado===‘llamada’?‘📞’:v.resultado===‘no_contesta’?‘📵’:‘👋’;var nt=v.notas||v.nota||’’;return ic+’ <b>’+(v.clienteNombre||v.cliente||’’)+’</b>’+(nt?’<br/><span style="color:#64748B;font-size:11px;padding-left:24px">’+nt+’</span>’:’’);});
var logrosAyer=[];
if(myEDoneAyer.length>0)myEDoneAyer.forEach(function(e){logrosAyer.push(‘✅ Estrategia: <b>’+(e.cliente||e.clienteNombre||’’)+’</b>’);});
if(myMDoneAyer.length>0)myMDoneAyer.forEach(function(m){logrosAyer.push(‘📦✅ Muestra OK: <b>’+(m.prod||m.producto||’’)+’</b>’);});
if(myMKOAyer.length>0)myMKOAyer.forEach(function(m){logrosAyer.push(‘📦❌ Muestra KO: <b>’+(m.prod||m.producto||’’)+’</b>’);});
if(myTDoneAyer.length>0)myTDoneAyer.forEach(function(t){logrosAyer.push(‘✅ Tarea: <b>’+(t.titulo||t.texto||’’)+’</b>’);});
var opoAyer=myOActAyer.map(function(a){return ‘💼 <b>’+a.nombre+’</b>’+(a.texto?’ — ‘+a.texto:’’);});
var incAyer=myIAyer.map(function(x){return ‘🚨 <b>’+(x.tipo||’’)+’</b> ‘+(x.clienteNombre||’’)+’: ‘+(x.descripcion||x.titulo||’’).substring(0,80);});
var pendItems2=[];
if(myHitos.length>0)myHitos.forEach(function(h){pendItems2.push((h.vencido?‘⚠️’:‘📍’)+’ <b>’+h.hito+’</b> — ‘+h.proy+(h.fecha?’ (’+(h.vencido?‘VENCIDO: ‘:’’)+h.fecha+’)’:’’));});
var sub=‘📋 Diario ‘+nm+’ — ‘+ayerStr+’: ‘+myVAyer.length+’ visitas, ‘+vPAyer+’ pedidos’;
var body=hdr(’#1E3A5F’,‘📋 Actividad de Ayer — ‘+nm,ayerStr+’ — Semana ‘+sem)+’<div style="padding:20px 24px;border:1px solid #E2E8F0;border-top:none;border-radius:0 0 14px 14px">’+’<table width="100%" cellspacing="6" style="border-collapse:separate;margin-bottom:16px"><tr>’+kpi(myVAyer.length,‘Visitas ayer’,’#22C55E’)+kpi(vPAyer,‘Pedidos’,’#3B82F6’)+kpi(totalAyer,‘Gestiones’,’#F59E0B’)+kpi(myVSem.length,‘Sem. acum.’,’#1E3A5F’)+’</tr></table>’+blk(’#F0FDF4’,’#22C55E’,‘Visitas ayer (’+myVAyer.length+’)’,vlAyer)+blk(’#EFF6FF’,’#1E3A5F’,‘Oportunidades ayer (’+myOActAyer.length+’)’,opoAyer)+blk(’#DBEAFE’,’#3B82F6’,‘Logros ayer (’+logrosAyer.length+’)’,logrosAyer)+blk(’#FEE2E2’,’#EF4444’,‘Incidencias ayer (’+myIAyer.length+’)’,incAyer)+sep(‘⏳ Pendiente esta semana’)+blk(’#FFF7ED’,’#F59E0B’,‘Hitos (’+myHitos.length+’) + Tareas (’+myT.length+’)’,pendItems2.concat(myT.slice(0,5).map(function(t){return ‘📌 <b>’+(t.titulo||t.texto||’’)+’</b>’+(t.vence?’ — Vence: ‘+t.vence:’’);})))+blk(’#DBEAFE’,’#3B82F6’,‘Estrategias en curso (’+myE.length+’)’,myE.slice(0,3).map(function(x){return ‘🎯 <b>’+(x.cliente||x.clienteNombre||’’)+’</b>: ‘+(x.texto||x.objetivo||’’).substring(0,60);}))+blk(’#F5F3FF’,’#7C3AED’,‘Pipeline (’+myO.length+’ — ‘+pp+‘€)’,myO.slice(0,3).map(function(x){return ‘💼 <b>’+(x.nombre||x.cliente||’’)+’</b> — ‘+(parseInt(x.valor)||0)+‘€’;}))+ft+’</div>’;
queue.unshift({to:pu.email,subject:sub,html:’<div style="font-family:sans-serif;max-width:560px;margin:0 auto;background:#fff;border-radius:14px;overflow:hidden">’+body+’</div>’});
}
...
var tituloDir=esViernes?‘📊 Resumen Semanal Dirección’:‘📋 Actividad de Ayer — Dirección’;
var subDir=esViernes?‘Direccion Sem.’+sem+’: ‘+aV.length+’ visitas, ‘+aP+‘€’:‘Diario Direccion — ‘+ayerStr+’: ‘+aVAyer.length+’ visitas’;
var vendorBlocks=’’;
...
var eq=ag.equipo||’’;var eqColor=eq===‘WIKUK’?’#166534’:eq===‘INTERKEY’?’#92400E’:’#1E3A5F’;
...
vendorBlocks+=’<div style="padding:12px 16px;background:'+eqColor+'08;border-bottom:1px solid #E2E8F0"><span style="font-size:14px;font-weight:800;color:#0F172A">’+anm+’</span>’+eqBadge;
vendorBlocks+=’ <span style="padding:3px 10px;border-radius:99px;background:#22C55E15;color:#22C55E;font-size:11px;font-weight:700;margin-left:8px">’+agV.length+’ vis</span>’;
...
vendorBlocks+=’</div><div style="padding:10px 16px">’;
...
var body2=hdr(’#0F172A’,tituloDir,(esViernes?‘Semana ‘+sem:‘Ayer: ‘+ayerStr)+’ — ‘+hoyStr)+’<div style="padding:20px 24px;border:1px solid #E2E8F0;border-top:none;border-radius:0 0 14px 14px">’+’<table width="100%" cellspacing="6" style="border-collapse:separate;margin-bottom:16px"><tr>’+kpi(visSource.length,esViernes?‘Visitas sem.’:‘Visitas ayer’,’#22C55E’)+kpi(aI.length,‘Incidencias’,’#EF4444’)+kpi(aO.length+’ (’+aP+‘€)’,‘Pipeline’,’#3B82F6’)+kpi(aE.length,‘Estrategias’,’#7C3AED’)+’</tr></table>’+sep(‘👥 Detalle por vendedor’+(esViernes?’ — semana completa’:’ — ayer’))+vendorBlocks+blk(’#FEF3C7’,’#F59E0B’,‘Tareas pendientes (’+aT.length+’)’,aT.slice(0,8).map(function(t){var tAg=portal.find(function(p){return ism(p.catalogoVendedor,rv(t.agente||t.agenteId||’’));});return ‘📌 <b>’+(t.titulo||t.texto||’’)+’</b>’+(tAg?’ — ‘+tAg.nombre:’’);}))+blk(’#FEE2E2’,’#EF4444’,‘Incidencias abiertas (’+aI.length+’)’,aI.slice(0,6).map(function(x){return ‘🚨 <b>’+(x.tipo||’’)+’</b> ‘+(x.clienteNombre||’’)+’: ‘+(x.descripcion||x.titulo||’’).substring(0,80);}))+ft+’</div>’;
...
return res.status(200).json({ok:true,sent:sent,queued:queue.length,tipo:esViernes?‘semanal’:‘diario’,dia:hoy.toISOString(),subjects:queue.map(function(q){return q.subject;}),errors:errors});
