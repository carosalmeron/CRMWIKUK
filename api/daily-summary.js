...
if (v.mapValue) {
var o = {};
Object.entries(v.mapValue.fields || {}).forEach(function(e) { o[e[0]] = pv(e[1]); });
return o;
}
return ‘’;
}

async function fbR(c) {
try {
var d = await httpGet(FB + ‘/’ + c + ‘?pageSize=500’);
...
function hdr(bg, t, s) {
return ‘<div style="background:' + bg + ';color:#fff;padding:20px 24px;border-radius:14px 14px 0 0"><h2 style="margin:0 0 4px;font-size:18px">’ + t + ‘</h2><p style="margin:0;font-size:13px;opacity:.7">’ + s + ‘</p></div>’;
}

function blk(bg, b, t, items) {
if (!items || items.length === 0) return ‘’;
return ‘<div style="margin-bottom:16px;padding:12px 16px;background:' + bg + ';border-radius:10px;border-left:4px solid ' + b + '"><p style="margin:0 0 6px;font-size:13px;font-weight:700;color:' + b + '">’ + t + ‘</p>’ + items.map(function(x) { return ‘<p style="margin:4px 0;font-size:12px;color:#475569;line-height:1.5">’ + x + ‘</p>’; }).join(’’) + ‘</div>’;
}

function kpi(v, l, c) {
return ‘<td style="background:' + c + '15;border-radius:10px;padding:12px;text-align:center"><p style="margin:0;font-size:20px;font-weight:800;color:' + c + '">’ + v + ‘</p><p style="margin:2px 0 0;font-size:10px;color:#64748B">’ + l + ‘</p></td>’;
}

function sep(t) {
return ‘<div style="margin:20px 0 12px;padding-bottom:8px;border-bottom:2px solid #E2E8F0"><p style="margin:0;font-size:14px;font-weight:800;color:#0F172A">’ + t + ‘</p></div>’;
}
...
function fechaMatch(fecha, target) {
if (!fecha) return false;
var p = fecha.split(’/’);
if (p.length !== 3) return false;
var dd = parseInt(p[0]), mm = parseInt(p[1]), yy = parseInt(p[2]);
if (yy < 100) yy += 2000;
return dd === target.getDate() && (mm - 1) === target.getMonth() && yy === target.getFullYear();
}
...
var incDB = await fbR('incidencias');
  ...
  try {
    if (rol === 'crm_agente' || rol === 'agente') {
      ...
      if (esViernes) {
        ...
        var vl = myVSem.map(function(v) {
          var ic = v.resultado === 'pedido' ? '✅' : v.resultado === 'llamada' ? '📞' : '👋';
          var nt = v.notas || v.nota || '';
          return ic + ' <b>' + (v.clienteNombre || v.cliente || '') + '</b>' + (nt ? '<br/><span style="color:#64748B;font-size:11px;padding-left:24px">' + nt + '</span>' : '');
        });
        var logros = [];
        ...
        myMKO.forEach(function(m) { logros.push('📦❌ <b>' + (m.prod || m.producto || '') + '</b>'); });
        myTDone.forEach(function(t) { logros.push('✅ <b>' + (t.titulo || t.texto || '') + '</b>'); });
        var pendItems = [];
        myHitos.forEach(function(h) { pendItems.push((h.vencido ? '⚠️' : '📍') + ' <b>' + h.hito + '</b> — ' + h.proy); });

        var sub = '📊 Semanal ' + nm + ' Sem.' + sem + ': ' + myVSem.length + ' visitas, ' + vPSem + ' pedidos';
        var body = hdr('#1E3A5F', '📊 Resumen Semanal — ' + nm, 'Semana ' + sem + ' — ' + hoyStr) +
          '<div style="padding:20px 24px;border:1px solid #E2E8F0;border-top:none;border-radius:0 0 14px 14px">' +
          '<table width="100%" cellspacing="6" style="border-collapse:separate;margin-bottom:16px"><tr>' + kpi(myVSem.length, 'Visitas', '#22C55E') + kpi(vPSem, 'Pedidos', '#3B82F6') + kpi(myT.length, 'Tareas', '#F59E0B') + kpi(pp + '€', 'Pipeline', '#1E3A5F') + '</tr></table>' +
          blk('#F0FDF4', '#22C55E', 'Visitas (' + myVSem.length + ')', vl) +
          blk('#DBEAFE', '#3B82F6', 'Logros (' + logros.length + ')', logros) +
          blk('#FFF7ED', '#F59E0B', 'Pendiente', pendItems.concat(myT.slice(0, 5).map(function(t) { return '📌 <b>' + (t.titulo || t.texto || '') + '</b>'; }))) +
          blk('#FEE2E2', '#EF4444', 'Incidencias (' + myI.length + ')', myI.slice(0, 4).map(function(x) { return '<b>' + (x.tipo || '') + '</b> ' + (x.clienteNombre || ''); })) +
          blk('#F5F3FF', '#7C3AED', 'Oportunidades (' + myO.length + ')', myO.slice(0, 4).map(function(x) { return '<b>' + (x.nombre || x.cliente || '') + '</b> — ' + (parseInt(x.valor) || 0) + '€'; })) +
          ft + '</div>';
        ...
        var vlAyer = myVAyer.map(function(v) {
          var ic = v.resultado === 'pedido' ? '✅' : v.resultado === 'llamada' ? '📞' : '👋';
          var nt = v.notas || v.nota || '';
          return ic + ' <b>' + (v.clienteNombre || v.cliente || '') + '</b>' + (nt ? '<br/><span style="color:#64748B;font-size:11px;padding-left:24px">' + nt + '</span>' : '');
        });
        var logrosAyer = [];
        myEDoneAyer.forEach(function(e) { logrosAyer.push('✅ Estrategia: <b>' + (e.cliente || '') + '</b>'); });
        myMDoneAyer.forEach(function(m) { logrosAyer.push('📦✅ <b>' + (m.prod || m.producto || '') + '</b>'); });
        myMKOAyer.forEach(function(m) { logrosAyer.push('📦❌ <b>' + (m.prod || m.producto || '') + '</b>'); });
        myTDoneAyer.forEach(function(t) { logrosAyer.push('✅ <b>' + (t.titulo || t.texto || '') + '</b>'); });
        var opoAyer = myOActAyer.map(function(a) { return '💼 <b>' + a.nombre + '</b>' + (a.texto ? ' — ' + a.texto : ''); });
        var incAyer = myIAyer.map(function(x) { return '🚨 <b>' + (x.tipo || '') + '</b> ' + (x.clienteNombre || ''); });
        var pendItems2 = [];
        myHitos.forEach(function(h) { pendItems2.push((h.vencido ? '⚠️' : '📍') + ' <b>' + h.hito + '</b> — ' + h.proy); });

        var sub = '📋 Diario ' + nm + ' — ' + ayerStr + ': ' + myVAyer.length + ' visitas';
        var body = hdr('#1E3A5F', '📋 Actividad de Ayer — ' + nm, ayerStr + ' — Semana ' + sem) +
          '<div style="padding:20px 24px;border:1px solid #E2E8F0;border-top:none;border-radius:0 0 14px 14px">' +
          ...
          blk('#F0FDF4', '#22C55E', 'Visitas ayer (' + myVAyer.length + ')', vlAyer) +
          blk('#EFF6FF', '#1E3A5F', 'Oportunidades ayer (' + myOActAyer.length + ')', opoAyer) +
          blk('#DBEAFE', '#3B82F6', 'Logros ayer (' + logrosAyer.length + ')', logrosAyer) +
          blk('#FEE2E2', '#EF4444', 'Incidencias ayer (' + myIAyer.length + ')', incAyer) +
          sep('⏳ Pendiente esta semana') +
          blk('#FFF7ED', '#F59E0B', 'Hitos + Tareas', pendItems2.concat(myT.slice(0, 5).map(function(t) { return '📌 <b>' + (t.titulo || t.texto || '') + '</b>'; }))) +
      ...
      var tituloDir = esViernes ? '📊 Resumen Semanal' : '📋 Actividad de Ayer';
      var subDir = esViernes ? 'Direccion Sem.' + sem + ': ' + aV.length + ' visitas' : 'Diario Direccion — ' + ayerStr;

      var vendorBlocks = '';
      for (var a = 0; a < agentes.length; a++) {
        var ag = agentes[a];
        var avd = (ag.catalogoVendedor || '').toUpperCase();
        var anm = ag.nombre || ag.id;
        ...
        vendorBlocks += '<div style="padding:12px 16px;background:' + eqColor + '08;border-bottom:1px solid #E2E8F0"><b>' + anm + '</b>';
        ...
        agV.forEach(function(v) {
          var ic = v.resultado === 'pedido' ? '✅' : '👋';
          var nt = v.notas || v.nota || '';
          vendorBlocks += '<p style="margin:3px 0;font-size:12px;color:#475569">' + ic + ' <b>' + (v.clienteNombre || v.cliente || '') + '</b>' + (nt ? ' — ' + nt : '') + '</p>';
        });
        vendorBlocks += '</div></div>';
      }

      var body2 = hdr('#0F172A', tituloDir, (esViernes ? 'Semana ' + sem : 'Ayer: ' + ayerStr) + ' — ' + hoyStr) +
        '<div style="padding:20px 24px;border:1px solid #E2E8F0;border-top:none;border-radius:0 0 14px 14px">' +
        '<table width="100%" cellspacing="6" style="border-collapse:separate;margin-bottom:16px"><tr>' +
        kpi(visSource.length, esViernes ? 'Visitas sem.' : 'Visitas ayer', '#22C55E') +
        kpi(aI.length, 'Incidencias', '#EF4444') +
        kpi(aP + '€', 'Pipeline', '#3B82F6') +
        kpi(aO.length, 'Oportunidades', '#7C3AED') +
        '</tr></table>' +
        sep('👥 Por vendedor') + vendorBlocks + ft + '</div>';
