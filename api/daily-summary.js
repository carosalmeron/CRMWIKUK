...
agV.forEach(function(v){var ic=v.resultado===‘pedido’?‘✅’:‘👋’;var nt=v.notas||v.nota||’’;vendorBlocks+=’<p style="margin:3px 0;font-size:12px;color:#475569">’+ic+’ <b>’+(v.clienteNombre||v.cliente||’’)+’</b>’+(nt?’ — ‘+nt:’’)+’</p>’;});
vendorBlocks+=’</div></div>’;
}
var body2=hdr(’#0F172A’,tituloDir,(esViernes?‘Semana ‘+sem:‘Ayer: ‘+ayerStr)+’ — ‘+hoyStr)+’<div style="padding:20px 24px;border:1px solid #E2E8F0;border-top:none;border-radius:0 0 14px 14px">’+’<table width="100%" cellspacing="6" style="border-collapse:separate;margin-bottom:16px"><tr>’+kpi(visSource.length,esViernes?‘Visitas sem.’:‘Visitas ayer’,’#22C55E’)+kpi(aI.length,‘Incidencias’,’#EF4444’)+kpi(aP+‘€’,‘Pipeline’,’#3B82F6’)+kpi(aE.length,‘Estrategias’,’#7C3AED’)+’</tr></table>’+sep(‘👥 Por vendedor’)+vendorBlocks+ft+’</div>’;
...
}catch(err){return res.status(200).json({error:err.message,stack:err.stack});}
};
